...
class ResultTest {
    ...
    void testErrorResultWithData() {
        ...
        Result<String> result = Result.fail(400, errorDetails, errorDetails);
    ...
    void testRecordComponents() {
        ...
        Result<String> result = new Result<>(code, message, data, timestamp, traceId, success);
    ...
    void testIndependentTimestamps() {
        Result<String> result1 = Result.ok("data1");
        Result<String> result2 = Result.ok("data2");
    ...
    @Test
    @DisplayName("Result from BusinessException should preserve error info")
    void testResultFromBusinessException() {
        BusinessException ex = BusinessException.of(BusinessException.ErrorCode.USER_NOT_FOUND);
        Result<Void> result = Result.fail(ex);

        assertAll("Result from exception",
            () -> assertEquals(BusinessException.ErrorCode.USER_NOT_FOUND.getCode(), result.code()),
            () -> assertEquals(BusinessException.ErrorCode.USER_NOT_FOUND.getDefaultMessage(), result.message()),
            () -> assertEquals(ex.getTraceId(), result.traceId()),
